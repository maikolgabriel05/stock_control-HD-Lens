const getSales = () => ({
    ...
    async saveData() {
        for (const item of this.sales) {
            if (!item.product || !item.stock || !item.price || !this.selectedCustomer) {
                console.error('Todos os campos devem ser preenchidos.'); // Verifica campos obrigatórios
                return;
            }
            ...
            console.log('Enviando dados para o servidor:', {
                ...
                name: this.selectedCustomer, // Adiciona o nome do cliente
            }); // Adiciona console.log()

            const response = await fetch('http://localhost:8000/shopping/carts/', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({
                    name: parseInt(this.selectedCustomer),
                    product: parseInt(item.product),
                    stock: parseInt(item.stock),
                    price: parseFloat(item.price),
                })
            ...
            console.log('Resposta do servidor:', data); // Adiciona console.log()
        }
